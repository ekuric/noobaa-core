...
function get_bucket_changes_quota(req, bucket, quota, single_bucket_update, changes) {
    ...
    if (quota === null) {
        ...
        quota_event.desc = `Bucket quota was removed from ${bucket.name} by ${req.account && req.account.email.unwrap()}`;
    } else {
        ...
        quota_event.desc = `Quota of ${size_utils.human_size(quota.value)} was set on ${bucket.name} by ${req.account && req.account.email.unwrap()}`;
    }
...
function get_bucket_changes_spillover(req, bucket, update_request, tiering_policy, changes) {
    ...
    if (update_request.spillover) {
        desc = `Bucket ${bucket.name} spillover on resource ${update_request.spillover} was set by ${req.account && req.account.email.unwrap()}`;
    } else {
        desc = `Bucket ${bucket.name} spillover was turned off by ${req.account && req.account.email.unwrap()}`;
    }
    changes.events.push({
    ...
    return can_delete_bucket(req.system, bucket)
        .then(reason => {
            ...
            Dispatcher.instance().activity({
                ...
                actor: req.account && req.account._id,
                bucket: bucket._id,
                desc: `${bucket.name} was deleted by ${req.account && req.account.email.unwrap()}`,
            });
            return system_store.make_changes({
                remove: {
                    buckets: [bucket._id],
...
function delete_bucket_lifecycle(req) {
    ...
    return system_store.make_changes({
        ...
        .then(() => {
            let desc_string = [];
            desc_string.push(`lifecycle configuration rules were removed for bucket ${bucket.name} by ${req.account && req.account.email.unwrap()}`);
...
function set_bucket_lifecycle_configuration_rules(req) {
    ...
    return system_store.make_changes({
        ...
        .then(() => {
            let desc_string = [];
            desc_string.push(`${bucket.name} was updated with lifecycle configuration rules by ${req.account && req.account.email.unwrap()}`);
