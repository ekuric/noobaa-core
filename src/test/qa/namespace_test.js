...
(async function() {
    ...
    const connections_mapping = { AZURE: await blobops.AzureDefaultConnectionAsync, AWS: AWSDefaultConnection };
    ...
    async function add_and_remove_resources(clouds) {
        ...
        for (let cycle = 0; cycle < run_on_clouds.length; cycle++) {
            await update_read_write_and_check(run_on_clouds, noobaa_bucket_name,
                read_resources, namespace_mapping[run_on_clouds[0]].namespace);
