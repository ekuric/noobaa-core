...
const GRACE_PERIODS = {
    NO_GRACE: 60 * MINUTE,
    DEFAULT_GRACE_PERIOD: 60 * MINUTE,
    MEMBER_START_TIME_ON_HEALTHY_CLUSTER: 60 * MINUTE,
    AFTER_NO_SERVICE: 60 * MINUTE, // after returning from no service it takes longer for mongodb to be operational. wait up to 10 minutes
    FAIL_OVER_TIME: 60 * MINUTE,
    AGENTS_CONNECTION_TO_CLUSTER: 60 * MINUTE,
};
...
function clean_env() {
    ...
    return P.map(machines, machine_name => azf.deleteVirtualMachine(machine_name)
            .catch(err => console.error(`Can't delete old server ${err.message}`)))
...
async function end_test(is_successful) {
    //TODO: DZDZ resore clean before commit!!!
    // try {
    //     await clean_env();
    // } catch (err) {
...
async function main() {
    ...
    try {
        ...
        agents_names = _.times(agents_number, i => `${agents_prefix}${i}`);
