...
const AzureDefaultConnectionAsync = (async function() {
    ...
    return {
        ...
        endpoint_type: "AZURE",
        identity: azure_details.AZURE.identity,
        secret: azure_details.AZURE.secret
    };
}());

const blobServiceAsync = (async function() {
    return azure_storage.createBlobService(
        (await AzureDefaultConnectionAsync).identity,
        (await AzureDefaultConnectionAsync).secret,
        (await AzureDefaultConnectionAsync).endpoint);
}());
...
async function uploadRandomFileDirectlyToAzure(container, file_name, size, err_handler) {
    ...
    try {
        const blobService = await blobServiceAsync;
...
exports.AzureDefaultConnectionAsync = AzureDefaultConnectionAsync;
